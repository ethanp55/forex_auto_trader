...
class CurrentDataSequence:
    def __init__(self):
        self.current_sequences = {'EUR_USD': None, 'GBP_CHF': None, 'USD_CAD': None, 'AUD_USD': None, 'NZD_USD': None}
    ...
    def update_macd_crossover_current_data_sequence(self, currency_pair):
        ...
        candles, error_message = data_downloader.get_current_data(currency_pair, ['bid', 'ask'], 'M30')
        ...
        row = [curr_date, float(curr_candle.bid.o), float(curr_candle.bid.h), float(curr_candle.bid.l), float(curr_candle.bid.c),
               float(curr_candle.ask.o), float(curr_candle.ask.h), float(curr_candle.ask.l), float(curr_candle.ask.c)]
        np_data.append(row)
