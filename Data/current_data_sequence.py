...
class CurrentDataSequence:
    def __init__(self):
        ...
        self.stoch_macd_current_sequences = {'AUD_USD': None}
        self.stoch_macd_current_long_sequences = {'AUD_USD': None}
    ...
    def update_stoch_macd_current_data_sequence(self, currency_pair):
        ...
        current_time = (datetime.now(tz=tz.timezone('America/New_York')).replace(microsecond=0, second=0,
                        minute=current_minutes - current_minutes % 30) - timedelta(hours=hours)).strftime('%Y-%m-%d %H:%M:%S')
        current_time = datetime.strptime(current_time, '%Y-%m-%d %H:%M:%S')

        from_time = str(current_time - timedelta(hours=2000))
        to_time = str(current_time)
        ...
        candles, error_message = data_downloader.get_historical_data(
            currency_pair, ['bid', 'ask', 'mid'], 'M30', from_time, to_time)
        ...
        long_data_sequence = pd.DataFrame(np_data, columns=['Date', 'Bid_Open', 'Bid_High', 'Bid_Low', 'Bid_Close',
                                                            'Ask_Open', 'Ask_High', 'Ask_Low', 'Ask_Close', 'Mid_Open', 'Mid_High', 'Mid_Low', 'Mid_Close', 'Volume'])
        long_data_sequence.dropna(inplace=True)
        long_data_sequence.reset_index(drop=True, inplace=True)

        if long_data_sequence.shape[0] < 1000:
            print('Current sequence length is too small: ' +
                  str(long_data_sequence.shape[0]))
            return False

        long_data_sequence = self.data_formatter.format_stoch_macd_data(
            currency_pair, long_data_sequence)
        long_data_sequence.reset_index(drop=True, inplace=True)

        self.stoch_macd_current_long_sequences[currency_pair] = long_data_sequence
    ...
    def get_stoch_macd_sequence_for_pair(self, currency_pair):
        return self.stoch_macd_current_sequences[currency_pair], self.stoch_macd_current_long_sequences[currency_pair]
